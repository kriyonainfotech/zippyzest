    ...
    return (
      <Link to={`/category/showcategory/${category._id}`} key={index}>
        <li className='cate-box p-3 d-flex'>
          <img src={`${apiurl}/uploads/${category.image}?t=${new Date().getTime()}`} alt="category" className='cateimg' />
            ...
            <div className="row mt-4">

              {
                products && products.map((val,i)=>{
                    return(
            <div className="col-6 mt-4 col-md-3 col-lg-3" >
              <Link to={`/productview/${val._id}`}>
                <div className="product-box">
                  <div className="product-img">
                    <img src={`${apiurl}/uploads/${val.image}?t=${new Date().getTime()}`} alt="" />
                  </div>
                  ...
                  <div className="product-part d-flex justify-content-between">
                    <div className="pro-price">
                      <p className="m-0 main-text">₹{val.price}</p>
                      <p className='m-0 main-text'>{val.productQuantity} {val.productUnit}</p>
                    </div>
                    <div className="add-to-cart-btn d-flex">
                      <Link  onClick={() => handleCart(val._id,val.price)}  className="px-2">
